/**
 * postprocessing v7.0.0-beta.16 build Thu Feb 19 2026
...
var Pass = class _Pass extends EventDispatcher8 {
  ...
  syncDefaultBuffers() {
    ...
    const useSRGB = this.autoSRGB && !this.output.frameBufferPrecisionHigh && renderer.outputColorSpace === SRGBColorSpace;
...
function addGBufferDefaultOutput(shader) {
...
var ClearPass = class extends Pass {
   ...
   */
  clearBuffers(gl, textureIndices) {
    ...
    for (const entry of textureIndices) {
      ...
      if (!flags.gBuffer.has(entry[0]) || clearValue === void 0) {
        continue;
      }
      if (typeof clearValue === "number") {
        ...
        fv[0] = clearValue.length > 0 ? clearValue[0] : 0;
        fv[1] = clearValue.length > 1 ? clearValue[1] : 0;
        fv[2] = clearValue.length > 2 ? clearValue[2] : 0;
        fv[3] = clearValue.length > 3 ? clearValue[3] : 1;
      }
   ...
   */
  clear(clearColor = this.clearValues.color) {
    const renderer = this.renderer;
  ...
  constructor() {
    super({
      ...
      uniforms: {
        ...
        channelWeights: new Uniform3(null)
      }
    });
...
var GBufferShaderPlugin = class {
  ...
  applyTo(material) {
    ...
    material.onBeforeCompile = (shader, renderer) => {
      ...
      if (material instanceof ShaderMaterial5) {
...
var GeometryPass = class extends Pass {
  ...
  updateOutputBufferColorSpace() {
    ...
    const useSRGB = this.autoSRGB && !this.frameBufferPrecisionHigh && renderer.outputColorSpace === SRGBColorSpace2;
    for (const entry of this.textureConfigs) {
      if (entry[1].isColorBuffer === true && indices.has(entry[0])) {
        ...
        texture.colorSpace = useSRGB ? SRGBColorSpace2 : NoColorSpace;
        texture.needsUpdate = true;
      }
    }
  }
...
import { Timer, Vector2 as Vector24, Vector4 as Vector43 } from "three";
...
// src/shader-chunks/shaders/noise-pars.frag
var noise_pars_default = `uint uhash(uint x){x+=x>>11;x ^=x<<7;x+=x>>15;x ^=x<<5;x+=x>>12;x ^=x<<9;return x;}uint uhash(uvec2 p){uint x=p.x;x+=x>>11;x ^=x<<7;x+=p.y;x ^=x<<6;x+=x>>15;x ^=x<<5;x+=x>>12;x ^=x<<9;return x;}uint uhash(uvec3 p){uint x=p.x;x+=x>>11;x ^=x<<7;x+=p.y;x ^=x<<3;x+=p.z ^(x>>14);x ^=x<<6;x+=x>>15;x ^=x<<5;x+=x>>12;x ^=x<<9;return x;}float urand(uint h){const uint mantissaMask=0x007FFFFFu;const uint one=0x3F800000u;h&=mantissaMask;h|=one;float r2=uintBitsToFloat(h);return r2-1.0;}float urand(float p){return urand(uhash(floatBitsToUint(p)));}float urand(vec2 p){return urand(uhash(floatBitsToUint(p)));}float urand(vec3 p){return urand(uhash(floatBitsToUint(p)));}`;

// src/shader-chunks/shaders/normal-codec-pars.frag
var normal_codec_pars_default = `vec2 pp_encodeNormal(vec3 n){n/=(abs(n.x)+abs(n.y)+abs(n.z));n.xy=(n.z>=0.0)?n.xy:(1.0-abs(n.yx))*vec2(n.x>=0.0?1.0:-1.0,n.y>=0.0?1.0:-1.0);return n.xy;}vec3 pp_decodeNormal(vec2 f){vec3 n=vec3(f.x,f.y,1.0-abs(f.x)-abs(f.y));float t=clamp(-n.z,0.0,1.0);n.xy+=vec2(n.x>=0.0?-t:t,n.y>=0.0?-t:t);return normalize(n);}`;
...
var gbuffer_roughness_physical_default = `#ifdef out_ORM
...
var ShaderChunkExtensions = class _ShaderChunkExtensions {
  ...
  static register() {
    ...
    Object.assign(ShaderChunk, {
      ...
      "pp_gbuffer_default_output_fragment": gbuffer_default_output_default,
      "pp_gbuffer_normal_fragment": gbuffer_normal_default,
      "pp_gbuffer_occlusion_fragment": gbuffer_occlusion_default,
      "pp_gbuffer_roughness_fragment": gbuffer_roughness_default,
      "pp_gbuffer_metalness_fragment": gbuffer_metalness_default,
      "pp_gbuffer_emission_fragment": gbuffer_emission_default,
      "pp_input_buffer_pars_fragment": input_buffer_pars_default,
      "pp_normal_codec_pars_fragment": normal_codec_pars_default,
      "pp_noise_pars_fragment": noise_pars_default,
...
var CDLPreset = /* @__PURE__ */ ((CDLPreset2) => {
  CDLPreset2[CDLPreset2["DEFAULT"] = 0] = "DEFAULT";
  CDLPreset2[CDLPreset2["GOLDEN"] = 1] = "GOLDEN";
  CDLPreset2[CDLPreset2["PUNCHY"] = 2] = "PUNCHY";
  CDLPreset2[CDLPreset2["NEEDLE"] = 3] = "NEEDLE";
  CDLPreset2[CDLPreset2["SEPIA"] = 4] = "SEPIA";
  CDLPreset2[CDLPreset2["GRAYSCALE"] = 5] = "GRAYSCALE";
  return CDLPreset2;
})(CDLPreset || {});
...
var HalftoneShape = /* @__PURE__ */ ((HalftoneShape2) => {
  ...
  HalftoneShape2[HalftoneShape2["HATCH"] = 2] = "HATCH";
  HalftoneShape2[HalftoneShape2["SQUARE"] = 3] = "SQUARE";
...
var mix_default = `vec4 blend(const in vec4 dst,const in vec4 src,const in float opacity){return mix(dst,src,opacity);}`;
...
import { Color as Color3, Uniform as Uniform5, Vector4 as Vector44 } from "three";
...
var ASCIIEffect = class extends Effect {
  ...
  constructor({
    ...
    uniforms.set("cellCount", new Uniform5(new Vector44()));
...
#endif
uniform float intensity;vec4 mainImage(const in vec4 inputColor,const in vec2 uv,const in GData gData){return texture(map,uv)*intensity;}`;
...
import { Uniform as Uniform10, Vector2 as Vector26 } from "three";

// src/effects/shaders/halftone.frag
var halftone_default = `#define SHAPE_DOT 1
...
var HalftoneEffect = class extends Effect {
  ...
  constructor({
    ...
    const uniforms = this.input.uniforms;
    uniforms.set("invRadius", new Uniform10(1));
    uniforms.set("rotation", new Uniform10(new Vector26()));
    uniforms.set("bias", new Uniform10(bias));
    this._radius = radius;
  ...
  set samples(value) {
    ...
    this.input.defines.set("INV_SAMPLES_SQ", (1 / (value * value)).toFixed(9));
    this.setChanged();
  }
  get radius() {
    return this._radius;
  ...
  set rotation(value) {
    const rotation = this.input.uniforms.get("rotation").value;
    rotation.set(Math.sin(value), Math.cos(value));
    this._rotation = value;
  }
  get bias() {
    return this.input.uniforms.get("bias").value;
  }
  set bias(value) {
  ...
  onResolutionChange() {
    ...
    this.input.uniforms.get("invRadius").value = 1 / Math.max(r, 1e-9);
  }
};

// src/effects/LensDistortionEffect.ts
import { Uniform as Uniform11, Vector2 as Vector27 } from "three";
...
var LensDistortionEffect = class extends Effect {
   ...
   */
  constructor({
    distortion = new Vector27(0, 0),
    principalPoint = new Vector27(0, 0),
    focalLength = new Vector27(1, 1),
    skew = 0
  } = {}) {
...
var LUT3DEffect = class extends Effect {
  ...
  constructor({
    ...
    uniforms.set("scale", new Uniform13(new Vector33()));
    uniforms.set("offset", new Uniform13(new Vector33()));
...
var noise_default = `uniform float page;vec4 mainImage(const in vec4 inputColor,const in vec2 uv,const in GData gData){vec3 p=vec3(uv,SEED+page);
#ifdef RGB
vec3 pR=p;vec3 pG=vec3(p.x+1.0,p.y,p.z);vec3 pB=vec3(p.x,p.y+1.0,p.z);vec3 noise=vec3(urand(pR),urand(pG),urand(pB));
#else
...
var NoiseEffect = class extends Effect {
  ...
  constructor({
    ...
    this.blendMode.blendFunction = new AddBlendFunction();
    this.input.uniforms.set("page", new Uniform14(0));
...
import { Uniform as Uniform15, Vector2 as Vector28 } from "three";
...
var ScanlineEffect = class extends Effect {
  ...
  constructor({
    ...
    const uniforms = this.input.uniforms;
    uniforms.set("params", new Uniform15(new Vector28(offset, 0)));
...
import { EventDispatcher as EventDispatcher12, MathUtils, Uniform as Uniform20, Vector3 as Vector34 } from "three";
var ColorDecisionList = class extends EventDispatcher12 {
  /**
  ...
  constructor() {
    super();
    this.uniform = new Uniform20({
      slope: new Vector34(1, 1, 1),
      offset: new Vector34(0, 0, 0),
      power: new Vector34(1, 1, 1),
      saturation: 1
    });
    this._enabled = true;
  }
  ...
  applyPreset(preset) {
    switch (preset) {
      case 0 /* DEFAULT */:
        this.slope.set(1, 1, 1);
        this.offset.set(0, 0, 0);
        this.power.set(1, 1, 1);
        this.saturation = 1;
        break;
      case 1 /* GOLDEN */:
        this.slope.set(1, 0.9, 0.5);
        this.offset.set(0, 0, 0);
...
var tone_mapping_default = `#ifdef USE_CDL
...
var ToneMappingEffect = class extends Effect {
   ...
   * @param options - The options.
   */
  constructor({ toneMapping = 4 /* AGX */, cdlPreset = null } = {}) {
    super("ToneMappingEffect");
    this.fragmentShader = tone_mapping_default.replace(
      ...
      "#include <tonemapping_pars_fragment>",
      ShaderChunk2.tonemapping_pars_fragment.replace(
...
import { Color as Color5, Uniform as Uniform21, Vector2 as Vector29 } from "three";
...
var VignetteEffect = class extends Effect {
  ...
  constructor({
    ...
    const uniforms = this.input.uniforms;
    uniforms.set("offsetFeather", new Uniform21(new Vector29(offset, feather)));
    uniforms.set("color", new Uniform21(new Color5(color2)));
  }
  get technique() {
...
var AdaptiveLuminanceMaterial = class extends FullscreenMaterial {
  ...
  constructor() {
    super({
      ...
      uniforms: {
        luminanceBuffer0: new Uniform22(null),
        luminanceBuffer1: new Uniform22(null),
        minLuminance: new Uniform22(0.01),
        deltaTime: new Uniform22(0),
        tau: new Uniform22(1)
      }
    });
  }
...
#endif
if(coc==0.0){out_Color=texture(inputBuffer,vUv);return;}
#ifdef FOREGROUND
...
var BokehMaterial = class extends FullscreenMaterial {
  ...
  constructor(fill = false, foreground = false) {
    super({
      ...
      uniforms: {
        cocBuffer: new Uniform23(null),
        kernel64: new Uniform23(null),
        kernel16: new Uniform23(null),
  ...
  constructor({ bilateral = false, kernelSize = 5 } = {}) {
    super({
      ...
      uniforms: {
        ...
        normalDepthBuffer: new Uniform24(null),
...
#include <pp_depth_utils_pars_fragment>
#include <pp_world_utils_pars_fragment>
uniform float focusDistance;uniform float focusRange;in vec2 vUv;void main(){float depth=readDepth(vUv);vec3 viewPosition=getViewPosition(vUv,depth);float distance=getDistance(viewPosition);float signedDistance=distance-focusDistance;float magnitude=smoothstep(0.0,focusRange,abs(signedDistance));out_Color.rg=magnitude*vec2(step(signedDistance,0.0),step(0.0,signedDistance));}`;
  ...
  constructor() {
    super({
      ...
      uniforms: {
        ...
        focusDistance: new Uniform26(0),
        focusRange: new Uniform26(0)
      }
    });
  }
  /**
...
import { Uniform as Uniform27, Vector2 as Vector210 } from "three";
  ...
  constructor() {
    super({
      ...
      uniforms: {
        ...
        texelPosition: new Uniform27(new Vector210())
      }
    });
  }
  /**
  ...
  constructor() {
    super({
      ...
      uniforms: {
        ...
        normalBuffer: new Uniform28(null)
      }
    });
  }
  /**
...
var EffectMaterial = class extends FullscreenMaterial {
  ...
  constructor() {
    super({
      ...
      uniforms: {
        gBuffer: new Uniform29(null),
        time: new Uniform29(0)
      }
    });
    this.shaderDataTracker = new ShaderDataTracker();
...
import { Uniform as Uniform30, Vector2 as Vector211 } from "three";
...
var GaussianBlurMaterial = class extends FullscreenMaterial {
  ...
  constructor({ kernelSize = 35 } = {}) {
    super({
      ...
      uniforms: {
        direction: new Uniform30(new Vector211()),
        kernel: new Uniform30(null),
...
var KawaseBlurMaterial = class extends FullscreenMaterial {
  ...
  constructor() {
    super({
      ...
      uniforms: {
        scale: new Uniform31(1),
        kernel: new Uniform31(0)
      }
    });
...
import { Uniform as Uniform32, UnsignedByteType as UnsignedByteType8 } from "three";
...
var MaskMaterial = class extends FullscreenMaterial {
  ...
  constructor(maskTexture = null) {
    super({
      ...
      uniforms: {
        maskTexture: new Uniform32(maskTexture),
        strength: new Uniform32(1)
      }
    });
  }
  /**
...
import { Uniform as Uniform33, Vector2 as Vector212, Vector4 as Vector45 } from "three";
...
var TiltShiftBlurMaterial = class extends KawaseBlurMaterial {
  ...
  constructor({
    ...
    this.uniforms.aspect = new Uniform33(1);
    this.uniforms.rotation = new Uniform33(new Vector212());
    this.uniforms.maskParams = new Uniform33(new Vector45());
    this._offset = offset;
...
var EffectMaterialManager = class _EffectMaterialManager {
  ...
  createMaterial(effects) {
    ...
    if (data.colorSpace === SRGBColorSpace5) {
      ...
      data.shaderParts.set("$FRAGMENT_MAIN_IMAGE" /* FRAGMENT_MAIN_IMAGE */, fragmentMainImage + "color0 = sRGBTransferEOTF(color0);\n	");
    }
...
var EffectPass = class extends Pass {
  ...
  set subpasses(value) {
    ...
    this.input.gBuffer.add("Color" /* COLOR */);
    for (const effect of super.subpasses) {
...
var DebugTools = class {
   ...
   */
  static analyzePipeline(pipeline) {
